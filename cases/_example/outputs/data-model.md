...
- **Embed detail lines:** `dataModelingNotes` and legacy `findByKey` confirm detail lines are read with the parent claim on every hot-path query (LEFT JOIN). `findDetailLines` projects from the same parent document; lines are not queried at volume without their parent. Embedding eliminates the JOIN and satisfies the 50 ms SLA with a single `find_one`.
- **Separate release rows:** Release rows are keyed by a distinct composite natural key `(cKey, cKey_Type, cCopy_Number)` and queried independently via `findRelease`. Storing them in `client_release_table` avoids bloating history documents (8M release rows vs 12M history docs) and supports direct composite lookups without loading parent aggregates.
...
|------------|-------|---------------|------------|
| `client_document_history` | `{ cKey: 1 }` unique | `idx_history_ckey` / PK | `findByKey`, `findDetailLines` |
| `client_document_history` | `{ cStatus: 1 }` | *(added for DAO)* | `countByStatus` — status-only count not indexed in relational DDL |
| `client_release_table` | `{ cKey: 1, cKey_Type: 1, cCopy_Number: 1 }` unique | `idx_release_composite` | `findRelease` |

- **No index on embedded `detailLines`:** relational `idx_detail_ckey` is subsumed by parent `cKey` lookup when lines are embedded.
- **No redundant prefix compounds:** the release composite index covers all prefix lookups on `(cKey)`, `(cKey, cKey_Type)`, and the full natural key; no shorter duplicate compounds on the same fields.

## 8. Rationale

Three relational tables collapse to two MongoDB collections. Embedding detail lines matches the dominant access pattern (parent + all lines in one round trip) and aligns with intake guidance that detail lines are always co-read with the parent. Release rows remain in a separate collection to support composite natural-key lookups and an independent row lifecycle without inflating history documents.

## 9. Assumptions

- Average 5 detail lines per claim document at production scale (60M / 12M), per `intake.json` `productionRowCounts`.
- Release rows are a separate bounded collection; not embedded in history documents, per composite-key access pattern in `dataModelingNotes` and legacy `findRelease`.
- `cStatus` single-field index is added for `countByStatus` dashboard queries; relational DDL only defines `cKey` on history and does not index status.
- `cKey` is stored as a field with a unique index (not MongoDB `_id`), preserving natural-key parity with legacy JDBC.

## 10. Approval

- Status: **approved**
- Approved at: 2026-07-14T20:06:00Z
