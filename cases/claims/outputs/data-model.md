...
- **Embed detail lines:** `dataModelingNotes` state that detail lines are read with the parent claim document on every hot-path query. Detail lines are not queried at volume without their parent. Embedding eliminates the relational JOIN and satisfies the 50 ms SLA with a single `find_one` on `cKey`.
- **Separate release rows:** Release rows are keyed by a distinct composite natural key `(cKey, cKey_Type, cCopy_Number)` and are accessed independently. Storing them in `client_release_table` avoids bloating history documents (8M release rows vs 12M history docs) and supports direct composite lookups without loading parent aggregates.
...
| `client_release_table` | `{ cKey: 1, cKey_Type: 1, cCopy_Number: 1 }` unique | `idx_release_composite` | Composite natural-key lookup |
...
- Status: **approved**
- Approved at: 2026-07-22T16:53:37Z
