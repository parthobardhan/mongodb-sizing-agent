...
- **Embed detail lines:** `dataModelingNotes` state that detail lines are read with the parent claim document on every hot-path query. Lines are not queried at volume without their parent. Embedding eliminates the relational JOIN and satisfies the 50 ms SLA with a single `find_one`.
- **Separate release rows:** Release rows are keyed by a distinct composite natural key `(cKey, cKey_Type, cCopy_Number)` and are modeled independently of the parent history document. Storing them in `client_release_table` avoids bloating history documents (8M release rows vs 12M history docs) and supports direct composite lookups without loading parent aggregates.
...
|------------|-------|-------------------|------------|
| `client_document_history` | `{ cKey: 1 }` unique | `idx_history_ckey` / PK | Primary lookup by claim key |
...
- `cKey` is stored as a field with a unique index (not MongoDB `_id`), preserving natural-key parity with the relational schema.

## 10. Approval

- Status: **pending**
- Awaiting user review before seed generation and sizing.
