#!/usr/bin/env python3
"""Seed sizing_claims_document_history with 500 docs per top-level collection."""
...
TEMPLATE_HISTORY = {
    ...
    "cStatus": "A",
    "cCreated_Date": datetime.now(timezone.utc),
    "detailLines": [
        {
            "cLine_Number": 1,
            "cClaim_Number": "CLM-000001",
...
TEMPLATE_RELEASE = {
    ...
    "cKey_Type": "01",
    "cCopy_Number": 1,
    "cRelease_Date": datetime.now(timezone.utc),
}
...
def randomize_history(doc: dict, i: int, rng: random.Random) -> dict:
    ...
    out["cStatus"] = rng.choice(["A", "B", "C", "D"])
    out["cCreated_Date"] = datetime.now(timezone.utc) - timedelta(days=rng.randint(0, 365))
    lines = []
...
def randomize_release(doc: dict, i: int, rng: random.Random) -> dict:
    ...
    out["cRelease_Date"] = datetime.now(timezone.utc) - timedelta(days=rng.randint(0, 180))
    return out
...
def seed(uri: str, clear: bool) -> None:
    ...
    if clear:
        for name in (COL_HISTORY, COL_RELEASE):
            db[name].drop()

    history_docs = [randomize_history(TEMPLATE_HISTORY, i, rng) for i in range(1, RECORDS + 1)]
    release_docs = [randomize_release(TEMPLATE_RELEASE, i, rng) for i in range(1, RECORDS + 1)]

    db[COL_HISTORY].insert_many(history_docs)
    db[COL_RELEASE].insert_many(release_docs)
    client.close()
    print(f"Inserted {RECORDS} documents into {COL_HISTORY} and {COL_RELEASE} on {DB_NAME}")
